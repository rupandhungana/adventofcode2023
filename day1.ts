const day1 = () => {
  const dataTest = `51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
5cfprzgxtf3465five`;
  ...
  dataTest.split("\n").map((line) => {
    ...
    console.log(`${line} :: ${Number(twoNumbers.join(""))}`);
  });
  console.log(`The total sum of calibration values is ${sum}`);
};
...
7pqrstsixteen`;
  ...
  input.split("\n").map((data) => {
    ...
    let twoNumbers = [numbers[0], numbers[numbers.length - 1]];
    sum += Number(twoNumbers.join(""));
    console.log(Number(twoNumbers.join("")));
  });
};
day2();
