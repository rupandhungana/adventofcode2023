const part1 = () => {
  const inputs = `51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
...
5cfprzgxtf3465five`;
  ...
  let sum: number = 0;
  inputs.split("\n").map((line) => {
    const strArry = line.split("");
    let numbers: number[] = [];
    for (let i = 0; i < strArry.length; i++) {
      if (!isNaN(Number(strArry[i]))) {
        numbers.push(Number(strArry[i]));
      }
    }
    let twoNumbers = [numbers[0], numbers[numbers.length - 1]];
    sum += Number(twoNumbers.join(""));
  });
  console.log(`Part 1 Value:  ${sum}`);
};

const part2 = () => {
  const input = `51591twosix4dhsxvgghxq
425nine
llvmhjtr8nbbhrfone
lpbjvpbtdfvtxtdvkpjs7qrvddkzmjtlqtg
3sixnineseven
...
5cfprzgxtf3465five`;
  ...
  input.split("\n").map((data) => {
    const toNumbers = data
      ...
      .replaceAll("nine", "n9e")
      .replaceAll("oneight", "o18t")
      .replaceAll("threeight", "t38t")
      .replaceAll("fiveight", "f58t")
      .replaceAll("sevenine", "s79e")
      .replaceAll("nineight", "n98t")
      .replaceAll("twoneight", "t218t")
      .replaceAll("twone", "t21e")
      .split("");
    let numbers: number[] = [];
    for (let i = 0; i < toNumbers.length; i++) {
      if (!isNaN(Number(toNumbers[i]))) {
        numbers.push(Number(toNumbers[i]));
      }
    }
    let twoNumbers = [numbers[0], numbers[numbers.length - 1]];
    console.log(`${data} ===============> ${twoNumbers}`);
    sum += Number(twoNumbers.join(""));
  });
  console.log(`Part two value: ${sum}`);
};
part1();
part2();
